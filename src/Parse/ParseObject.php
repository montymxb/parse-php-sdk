...
class ParseObject implements Encodable
{
    ...
    public static function deleteAllObjects($className) {
        if(!isset($className)) {
            return;
        }
        // fetch and delete ALL objects of this class to free it up
        $query = new ParseQuery($className);
        $limit = 1000;
        $query->limit($limit);
        while($foundObjs = $query->find(true)) {
            ParseObject::destroyAll($foundObjs, true);
    ...
    public function getRelation($key, $className = null)
    {
        ...
        /* issue regarding not passing a classname with migrated parse data, estimatedData never contains our relation's className and as a result it is never set.
